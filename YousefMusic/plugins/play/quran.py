...
@app.on_message(filters.command(["القران"],""))
async def quran(c,msg):
    quran = json.loads(open("YousefMusic/assets/quran.json").read())["s"]
    keyboard = []
    list = []
    for i in range(1,11):
        if len(list) == 2:
